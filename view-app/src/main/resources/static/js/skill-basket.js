...
document.addEventListener('mouseout', function (e) {
  ...
  if (related && battery.contains(related)) return;
...
document.addEventListener('submit', function (e) {
  var form = e.target;
  if (!form.matches('.hb-skill-form')) return;
  e.preventDefault();
  var skillId = form.getAttribute('data-skill-id') || form.querySelector('[name="skillId"]').value;
  ...
  fetch('/basket/items', {
    ...
    body: 'skillId=' + encodeURIComponent(skillId) + '&level=' + encodeURIComponent(level)
  }).then(function (r) {
    if (!r.ok) throw new Error('basket add failed');
