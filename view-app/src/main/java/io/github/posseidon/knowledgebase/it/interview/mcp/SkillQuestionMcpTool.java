...
public class SkillQuestionMcpTool {
  ...
  @Tool(name = "find_questions_for_skill",
      description =
          "Match a skill name (e.g. from a job position description) to the closest skill "
  ...
  public SkillQuestionsResult findQuestionsForSkill(
    ...
    List<Question> questions = questionRepository.findBySkillIdAndLevel(skill.getId(),
        resolvedLevel.name());
    List<QuestionView> views = questions.stream().map(questionMapper::toView).toList();

    return new SkillQuestionsResult(true, skill.getId(), skill.getName(), resolvedLevel.name(),
        views);
  }
