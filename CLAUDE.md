...
and answers, ingests
them from JSON, lists them by topic/tag, answers natural-language questions over them with a local
LLM (RAG), and
...
- **Ollama** (local) for embeddings (`nomic-embed-text`, 768-dim) and chat (`llama3.1:8b` or
  `qwen2.5:7b`), accessed
  through Spring AI.
...
- `org.springframework.ai:spring-ai-starter-model-ollama` (provides `ChatModel`/
