...
public class MergeService {
        ...
        for (Question q : allQuestions) {
            ...
            for (Document doc : results) {
                ...
                try {
                    ...
                    if (questionRepository.existsById(candidateId)) {
                        candidates.add(new MergeCandidate(q.getId(), candidateId,
    ...
    public void merge(UUID targetId, UUID sourceId) {
        ...
        vectorStore.add(List.of(Document.builder()
                ...
                .metadata(Map.of(
                        "topics", target.getTopics().stream().map(Topic::getSlug).toList(),
                        "tags", target.getTags().stream().map(Tag::getName).toList(),
