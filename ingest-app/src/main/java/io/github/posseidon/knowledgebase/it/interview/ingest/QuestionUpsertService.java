...
/**
 * Single upsert-by-externalId/contentHash implementation shared by every ingestion entry
 * point (direct /ingest and inline questions nested in /interviews), so the dedup rules and
...
public class QuestionUpsertService {
    ...
    public Result upsert(List<QuestionDto> dtos) {
        if (dtos == null || dtos.isEmpty()) return Result.empty();
    ...
    /**
     * Matches one DTO to an existing question by externalId, falling back to contentHash,
     * falling back to creating a new one — then applies the DTO's fields onto it.
     */
    ...
    private Map<UUID, Set<String>> loadExistingAnswerHashes(List<ResolvedQuestion> resolved) {
        ...
        if (preExistingIds.isEmpty()) return Map.of();
    ...
    private List<Question> saveQuestions(List<ResolvedQuestion> resolved) {
        List<String> staleVectorIds = resolved.stream()
                ...
                .toList();
        if (!staleVectorIds.isEmpty()) vectorStore.delete(staleVectorIds);
    ...
    private int saveNewAnswers(List<QuestionDto> dtos, List<Question> saved,
        ...
        for (int i = 0; i < dtos.size(); i++) {
            ...
            for (AnswerDto answerDto : dto.answers()) {
                if (answerDto.content() == null || answerDto.content().isBlank()) continue;
